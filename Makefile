...
run:
	$(QEMU) -cdrom $(ISO_FILE) $(QEMU_OPT) -serial file:kernel.log  -device VGA -monitor stdio -m 8G

gdb: $(IMG_FILE)
	$(QEMU) -cdrom $(ISO_FILE) $(QEMU_OPT) -serial file:kernel.log  -device VGA -monitor stdio -d cpu_reset -D qemu.log -gdb tcp::1234 -S -m 8G
